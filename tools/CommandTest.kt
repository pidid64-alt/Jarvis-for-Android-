...
fun main() {
    ...
    println("== Поиск, Клод и презентации ==")
    // Сам поиск и Клод — сетевые, поэтому здесь проверяем разбор фразы и то,
    // что тяжёлая работа уходит в фон (Outcome.async), а не блокирует ответ.
    check("презентация: плашка ожидания", true,
        reply("поищи инфу про марс и сделай презентацию")?.startsWith("Принято, сэр."))
    check("презентация: работа уходит в фон", true,
        exec("поищи инфу про марс и сделай презентацию")?.async != null)
    check("презентация: тема в ответе", true,
        reply("сделай презентацию про древний рим на 10 слайдов")?.contains("древний рим"))
    check("презентация: число слайдов в ответе", true,
        reply("сделай презентацию про древний рим на 10 слайдов")?.contains("10 слайдов"))
    val quietSearch = exec("поищи информацию про рынок кофе")
    check("поиск без презентации — тихо", true, quietSearch?.async != null)
    check("поиск без презентации — без «ищу»", true, quietSearch?.reply.isNullOrBlank())
    check("презентация без темы — переспрос", true,
        reply("сделай презентацию")?.startsWith("Про что делать презентацию"))
    check("вопрос Клоду уходит в фон", true, exec("спроси у клода как настроить роутер")?.async != null)
    ...
    check("закрой это — больше не тишина", true, reply("закрой это") != null)
    check("закрой это — реакция", true,
        (reply("закрой это") ?: "").let { it.contains("спец") || it.startsWith("Закрываю") })
    check("назад — команда окна", true, reply("назад") != null)
    check("что такое — тихий поиск в фоне", true,
        exec("что такое квантовый компьютер")?.async != null)
    check("справка знает про закрой это", true,
        reply("что ты умеешь")?.contains("закрой это"))
